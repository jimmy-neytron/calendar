    ...
    <section class="entries-card panel">
      ...
      <div v-if="pagedEntries.length" class="project-entry-list">
        <article
          ...
          :key="entry.id"
          :class="{ 'is-editing': editingEntryId === entry.id }"
        >
          <form
            v-if="editingEntryId === entry.id"
            class="entry-edit"
            @submit.prevent="saveEntryEdit"
          >
            <div class="entry-edit__grid">
              <label>
                <span>Дата</span>
            ...
            <div class="entry-edit__actions">
              <button type="submit"><UiIcon name="check" /> Сохранить</button>
              <button type="button" @click="cancelEntryEdit"><UiIcon name="close" /> Отмена</button>
            </div>
          </form>
          <template v-else>
...
async function saveEntryEdit() {
  ...
  const result = await timeTrackingStore.updateEntry(editingEntryId.value, {
  ...
  notify(
    result.ok ? 'Запись обновлена' : result.message || 'Не удалось обновить запись',
    result.ok ? 'success' : 'danger',
  )
  if (result.ok) cancelEntryEdit()
}
