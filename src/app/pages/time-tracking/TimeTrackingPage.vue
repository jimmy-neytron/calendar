<template>
  <section class="time-page">
    ...
    <section class="time-entry-list-wrap">
      <TimeEntryList
        :entries="entries"
        :projects="projects"
        @remove="removeEntry"
    ...
    <UiModal v-model="isEntryModalOpen" title="Новая запись времени" eyebrow="Учёт времени" width="680px" :close-on-overlay="!entryLoading && !projectLoading">
      <TimeEntryComposer
        :projects="projects"
...
async function addEntry(payload: { projectId: string; date: string; minutes: number; note: string }) {
  ...
  if (result.ok) isEntryModalOpen.value = false
}
...
.time-hero{grid-template-columns:minmax(0,1fr) auto}.time-hero__actions{position:relative;z-index:1;display:flex;align-items:stretch;gap:8px}.time-entry-list-wrap{min-width:0}.time-entry-list-wrap>:deep(.time-entry-list){width:100%}@media(max-width:720px){.time-hero{grid-template-columns:1fr}.time-hero__actions{display:grid}.time-hero__total{justify-items:start}}
</style>
