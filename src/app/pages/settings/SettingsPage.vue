<template>
  <section class="settings-page">
    ...
    <section class="current-plan-card panel">
      <span class="current-plan-card__icon">
        <UiIcon :name="currentPlan.icon" />
      </span>
      <div class="current-plan-card__content">
        <div class="current-plan-card__title">
          <small>Текущий тариф</small>
          <strong>{{ currentPlan.name }}</strong>
        </div>
        <span class="current-plan-card__badge">
          <UiIcon name="check" /> Активен
        </span>
      </div>
      <div class="current-plan-card__meta">
        <span>
          <UiIcon name="grid" />
        ...
        <article class="setting-card setting-card--switches">
          ...
          <label class="setting-switch">
            <span>
              ...
              <small>Покупки, фильмы, семейное дерево, спорт и учёт времени. Когда выключено, разделы и связанные с ними события скрыты.</small>
            </span>
            <UiToggle
...
const workspaceLimitLabel = computed(() => {
  const limit = Number(currentPlan.value.workspaceLimit)
  if (limit === 1) return 'пространство'
  if (limit > 1 && limit < 5) return 'пространства'
  return 'пространств'
})
...
.current-plan-card{position:relative;display:grid;grid-template-columns:56px minmax(0,1fr) auto;align-items:center;gap:14px;overflow:hidden;padding:18px;border-color:color-mix(in srgb,var(--accent) 20%,var(--border-color));background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 9%,var(--panel-bg)),var(--panel-bg))}.current-plan-card::after{position:absolute;right:-48px;top:-64px;width:160px;height:160px;border:1px solid color-mix(in srgb,var(--accent) 16%,transparent);border-radius:50%;content:"";pointer-events:none}.current-plan-card__icon{position:relative;z-index:1;display:grid;place-items:center;width:56px;height:56px;border:1px solid color-mix(in srgb,var(--accent) 28%,var(--border-color));border-radius:14px;color:var(--accent);background:color-mix(in srgb,var(--accent) 12%,var(--control-bg));box-shadow:var(--shadow-sm);font-size:25px}.current-plan-card__content{position:relative;z-index:1;display:flex;align-items:center;gap:10px;min-width:0}.current-plan-card__title{min-width:0}.current-plan-card small,.current-plan-card strong{display:block}.current-plan-card small{color:var(--text-muted);font-size:9px;font-weight:850;letter-spacing:.12em;text-transform:uppercase}.current-plan-card strong{overflow:hidden;margin-top:2px;color:var(--text-primary);font-size:22px;line-height:1.1;text-overflow:ellipsis;white-space:nowrap}.current-plan-card__badge{display:inline-flex;align-items:center;gap:5px;border:1px solid color-mix(in srgb,var(--success) 26%,var(--border-color));border-radius:var(--radius-pill);padding:5px 8px;color:var(--success);background:color-mix(in srgb,var(--success) 8%,var(--control-bg));font-size:9px;font-weight:850;white-space:nowrap}.current-plan-card__meta{position:relative;z-index:1;display:flex;justify-content:flex-end}.current-plan-card__meta span{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--border-color);border-radius:10px;padding:9px 10px;color:var(--text-secondary);background:var(--card-soft);font-size:11px;font-weight:750;white-space:nowrap}.current-plan-card__meta svg{color:var(--accent)}
...
@keyframes savedPop{from{opacity:0;transform:translateY(-5px) scale(.94)}to{opacity:1;transform:none}}@media(max-width:860px){.release-feature-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.release-details ul{grid-template-columns:1fr}}@media(max-width:720px){.settings-hero{align-items:flex-start;padding:19px}.settings-hero__actions{display:grid}.current-plan-card{grid-template-columns:56px minmax(0,1fr)}.current-plan-card__content{align-items:flex-start;display:grid}.current-plan-card__meta{grid-column:1/-1;justify-content:flex-start}.calendar-settings{padding:15px}.settings-grid{grid-template-columns:1fr}.setting-card--large,.setting-card--switches{grid-column:auto}.view-options{grid-template-columns:1fr}.view-options button{min-height:92px;grid-template-columns:42px minmax(0,1fr);align-items:center}.view-options button>span{grid-row:1/3;margin:0}.theme-options{grid-template-columns:1fr}.release-cover{grid-template-columns:1fr}.release-feature-grid{grid-template-columns:1fr}}@media(max-width:480px){.settings-hero{display:grid}.settings-hero__actions{grid-template-columns:auto auto}.profile-settings{align-items:stretch;display:grid}.profile-settings :deep(.ui-button){width:100%}.current-plan-card{grid-template-columns:1fr}.current-plan-card__icon{width:50px;height:50px}.current-plan-card strong{white-space:normal}.calendar-settings__header{align-items:flex-start}.setting-switch{align-items:flex-start}.release-cover__copy h3{font-size:20px}}
</style>
