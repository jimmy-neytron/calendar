<template>
  <UiModal :model-value="modelValue" :title="coupon?.title || 'Купон'" :eyebrow="coupon?.merchant || 'Покажи на кассе'" width="520px" @update:model-value="emit('update:modelValue', $event)">
    <div v-if="coupon" class="coupon-code">
      ...
      <div v-if="coupon.codeType === 'qr'" class="coupon-code__canvas coupon-code__canvas--qr"><canvas ref="primaryCanvas" /><p v-if="renderError">{{ renderError }}</p></div>
      <div v-else-if="coupon.codeType === 'barcode'" class="coupon-code__barcodes">
        <section><small>{{ coupon.secondaryCodeValue ? 'Верхний штрихкод' : 'Штрихкод' }}</small><div class="coupon-code__canvas"><canvas ref="primaryCanvas" /></div></section>
        <section v-if="coupon.secondaryCodeValue"><small>Нижний штрихкод</small><div class="coupon-code__canvas"><canvas ref="secondaryCanvas" /></div></section>
        <p v-if="renderError">{{ renderError }}</p>
      </div>
      ...
      <button v-if="coupon.codeValue" type="button" class="coupon-code__copy" @click="copyCode"><UiIcon :name="copied ? 'check' : 'copy'" /> {{ copied ? 'Код скопирован' : coupon.secondaryCodeValue ? 'Скопировать оба кода' : 'Скопировать код' }}</button>
...
import type { Coupon, CouponBarcodeFormat } from '../../../types/coupon'
...
const primaryCanvas = ref<HTMLCanvasElement | null>(null)
const secondaryCanvas = ref<HTMLCanvasElement | null>(null)
...
watch(() => [props.modelValue, props.coupon?.id, props.coupon?.updatedAt] as const, async ([open]) => {
  ...
  await nextTick()
  if (!primaryCanvas.value) return
  try {
    if (props.coupon.codeType === 'qr') {
      qrcode(primaryCanvas.value, { bcid: 'qrcode', text: props.coupon.codeValue, scale: 5, padding: 3, backgroundcolor: 'ffffff' })
    } else {
      renderBarcode(primaryCanvas.value, props.coupon.codeValue, props.coupon.barcodeFormat)
      if (props.coupon.secondaryCodeValue && secondaryCanvas.value) {
        renderBarcode(secondaryCanvas.value, props.coupon.secondaryCodeValue, props.coupon.secondaryBarcodeFormat)
      }
    }
  } catch (reason) {
...
async function copyCode() {
  ...
  const values = [props.coupon.codeValue, props.coupon.secondaryCodeValue].filter(Boolean)
  try { await navigator.clipboard.writeText(values.join('\n')); copied.value = true } catch { copied.value = false }
}
...
function renderBarcode(target: HTMLCanvasElement, value: string, format: CouponBarcodeFormat) {
  const options = { bcid: format, text: value, scale: 3, height: 18, includetext: true, textxalign: 'center' as const, textsize: 10, padding: 4, backgroundcolor: 'ffffff' }
  const retailOptions = { ...options, height: 16, textsize: 9, textyoffset: -9, guarddescent: 2, paddingbottom: 10 }
  if (format === 'ean13') return ean13(target, retailOptions)
  if (format === 'ean8') return ean8(target, retailOptions)
  if (format === 'upca') return upca(target, retailOptions)
  return code128(target, options)
}
</script>

<style scoped>
.coupon-code{display:grid;justify-items:center;gap:12px}.coupon-code__summary{width:100%;display:grid;grid-template-columns:3px minmax(0,1fr) auto;align-items:center;gap:10px;border:1px solid var(--border-color);border-radius:11px;padding:11px;background:var(--control-bg)}.coupon-code__summary>i{width:3px;height:31px;border-radius:3px;background:var(--coupon-color)}.coupon-code__summary>div{display:grid;gap:2px}.coupon-code__summary small,.coupon-code__summary>span{color:var(--text-muted);font-size:8px}.coupon-code__summary strong{font-size:17px}.coupon-code__barcodes{width:100%;display:grid;gap:10px}.coupon-code__barcodes section{display:grid;gap:5px}.coupon-code__barcodes small{color:var(--text-muted);font-size:8px;font-weight:700}.coupon-code__barcodes>p{margin:0;color:#dc2626;font-size:10px}.coupon-code__canvas{width:100%;min-height:150px;display:grid;place-items:center;overflow:auto;border:1px solid #e5e7eb;border-radius:12px;padding:18px;background:#fff}.coupon-code__canvas--qr{width:min(100%,340px);min-height:190px;padding:22px}.coupon-code__canvas canvas{display:block;max-width:100%;height:auto}.coupon-code__canvas p{color:#dc2626;font-size:10px}.coupon-code__promo{width:100%;display:grid;justify-items:center;gap:7px;border:1px solid var(--border-color);border-radius:12px;padding:28px 16px;background:var(--control-bg)}.coupon-code__promo small{color:var(--text-muted);font-size:8px;text-transform:uppercase;letter-spacing:.1em}.coupon-code__promo strong{font-size:clamp(22px,5vw,31px);letter-spacing:.06em;overflow-wrap:anywhere}.coupon-code__empty{min-height:140px;display:grid;place-items:center;align-content:center;gap:9px;color:var(--text-muted);font-size:10px}.coupon-code__empty :deep(svg){color:var(--success);font-size:25px}.coupon-code__copy{width:100%;display:flex;align-items:center;justify-content:center;gap:6px;border:1px solid var(--border-color);border-radius:9px;padding:9px;color:var(--text-secondary);background:var(--control-bg);font-size:9px}.coupon-code__copy:hover{border-color:var(--border-strong);background:var(--control-bg-hover)}.coupon-code__terms{width:100%;border-top:1px solid var(--border-color);padding-top:11px;color:var(--text-muted);font-size:9px}.coupon-code__terms summary{color:var(--text-secondary);font-weight:700;cursor:pointer}.coupon-code__terms p{margin:7px 0 0;line-height:1.5}.coupon-code footer{width:100%;display:flex;justify-content:flex-end;gap:8px;border-top:1px solid var(--border-color);padding-top:12px}
</style>
