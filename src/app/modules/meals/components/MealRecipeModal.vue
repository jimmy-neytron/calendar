  ...
  >
    <form class="recipe-form" @submit.prevent="submit">
      <section class="recipe-basics">
        <UiInput v-model="form.title" label="Название" placeholder="Например, гречка с курицей" required />
      ...
      <section class="recipe-section">
        <header>
          <div>
            <h3>Ингредиенты</h3>
            <p>Необязательно. Если у всех продуктов есть данные, калории посчитаются автоматически.</p>
          </div>
        </header>

        <div class="ingredient-search">
          <UiInput v-model="foodQuery" placeholder="Название продукта или штрихкод" />
...
import type { FoodReference, MealIngredient, MealRecipe, MealType } from '../types/meals.types'
...
const localResults = computed(() => {
  ...
  return LOCAL_FOODS.filter((food) => normalize(food.name).includes(query)).slice(0, 8)
})
