export const UI_ICONS = Object.freeze({
  close: { paths: ['M6 6l12 12', 'M18 6L6 18'] },
  left: { paths: ['M15 18l-6-6 6-6'] },
  right: { paths: ['M9 18l6-6-6-6'] },
  up: { paths: ['M18 15l-6-6-6 6'] },
  down: { paths: ['M6 9l6 6 6-6'] },
  plus: { paths: ['M12 5v14', 'M5 12h14'] },
  minus: { paths: ['M5 12h14'] },
  trash: { paths: ['M4 7h16', 'M9 7V4h6v3', 'M7 7l1 13h8l1-13', 'M10 11v5', 'M14 11v5'] },
  copy: { paths: ['M8 8h11v11H8z', 'M5 16H4V5h11v1'] },
  edit: { paths: ['M4 20h4l11-11-4-4L4 16v4z', 'M13.5 6.5l4 4'] },
  notes: { paths: ['M5 3h14v18H5z', 'M8 8h8', 'M8 12h8', 'M8 16h5'] },
  book: { paths: ['M4 5a3 3 0 013-3h5v18H7a3 3 0 00-3 2V5z', 'M20 5a3 3 0 00-3-3h-5v18h5a3 3 0 013 2V5z'] },
  library: { paths: ['M4 4h5v16H4z', 'M9 6h5v14H9z', 'M15 5l4-1 3 15-4 1-3-15z', 'M5.5 8h2', 'M10.5 10h2'] },
  folder: { paths: ['M3 6h7l2 2h9v11H3V6z'] },
  compass: { paths: ['M15.5 8.5l-2 5-5 2 2-5 5-2z'], circles: [{ cx: 12, cy: 12, r: 9 }] },
  'check-circle': { paths: ['M8 12l3 3 6-7'], circles: [{ cx: 12, cy: 12, r: 9 }] },
  network: { paths: ['M12 7v4', 'M7 17h10', 'M7 17l5-6 5 6'], circles: [{ cx: 12, cy: 4, r: 3 }, { cx: 6, cy: 19, r: 3 }, { cx: 18, cy: 19, r: 3 }] },
  pin: { paths: ['M9 4h6', 'M10 4v5l-3 3v2h10v-2l-3-3V4', 'M12 14v7'] },
  target: { circles: [{ cx: 12, cy: 12, r: 9 }, { cx: 12, cy: 12, r: 5 }, { cx: 12, cy: 12, r: 1 }] },
  fire: { paths: ['M12 22c4 0 7-3 7-7 0-3-2-6-5-9 0 3-1 4-2 5-1-3-1-6 0-9-4 3-7 7-7 12 0 4 3 7 7 7z'] },
  trophy: { paths: ['M8 4h8v5a4 4 0 01-8 0V4z', 'M8 6H4v2a4 4 0 004 4', 'M16 6h4v2a4 4 0 01-4 4', 'M12 13v4', 'M8 21h8', 'M9 17h6v4H9z'] },
  lock: { paths: ['M6 10h12v10H6z', 'M8 10V7a4 4 0 018 0v3', 'M12 14v2'] },
  check: { paths: ['M5 12l4 4L19 6'] },
  calendar: { paths: ['M5 4h14a1 1 0 011 1v15H4V5a1 1 0 011-1z', 'M8 2v4', 'M16 2v4', 'M4 9h16'] },
  activity: { paths: ['M4 12h4l2-6 4 12 2-6h4'] },
  clock: { paths: ['M12 7v5l3 2'], circles: [{ cx: 12, cy: 12, r: 9 }] },
  filter: { paths: ['M4 6h16', 'M7 12h10', 'M10 18h4'] },
  refresh: { paths: ['M20 11a8 8 0 10-2 5', 'M20 5v6h-6'] },
  download: { paths: ['M12 3v12', 'M7 10l5 5 5-5', 'M5 21h14'] },
  search: { paths: ['M11 18a7 7 0 100-14 7 7 0 000 14z', 'M20 20l-4-4'] },
  heart: { paths: ['M20.8 4.6a5.5 5.5 0 00-7.8 0L12 5.7l-1.1-1.1a5.5 5.5 0 00-7.8 7.8L12 21l8.8-8.6a5.5 5.5 0 000-7.8z'] },
  sparkles: { paths: ['M12 3l1.3 4.2L17 9l-3.7 1.8L12 15l-1.3-4.2L7 9l3.7-1.8L12 3z', 'M19 15l.7 2.3L22 18l-2.3.7L19 21l-.7-2.3L16 18l2.3-.7L19 15z'] },
  sport: { paths: ['M6 8v8', 'M3 10v4', 'M18 8v8', 'M21 10v4', 'M6 12h12'] },
  chart: { paths: ['M5 20V10', 'M12 20V4', 'M19 20v-7'] },
  wallet: { paths: ['M4 6h14a2 2 0 012 2v11H4a2 2 0 01-2-2V6a2 2 0 012-2h12', 'M15 11h5v4h-5a2 2 0 010-4z'] },
  shopping: { paths: ['M5 8h14l-1 12H6L5 8z', 'M9 10V6a3 3 0 016 0v4'] },
  ticket: { paths: ['M4 6h16v4a2 2 0 000 4v4H4v-4a2 2 0 000-4V6z', 'M9 6v12'] },
  barcode: { paths: ['M4 5v14', 'M7 5v14', 'M10 5v14', 'M14 5v14', 'M17 5v14', 'M20 5v14'] },
  utensils: { paths: ['M7 3v7', 'M4 3v4a3 3 0 006 0V3', 'M7 10v11', 'M16 3v18', 'M16 3c3 2 4 5 4 8h-4'] },
  ruler: { paths: ['M5 3h14v18H5z', 'M9 6H5', 'M12 9H5', 'M9 12H5', 'M12 15H5', 'M9 18H5'] },
  hanger: { paths: ['M12 7a2.5 2.5 0 10-2.5-2.5', 'M12 7v2', 'M12 9L3 17v2h18v-2l-9-8z'] },
  camera: { paths: ['M4 7h4l2-3h4l2 3h4v12H4z'], circles: [{ cx: 12, cy: 13, r: 4 }] },
  home: { paths: ['M3 11l9-8 9 8', 'M5 10v10h14V10', 'M9 20v-6h6v6'] },
  user: { paths: ['M20 21a8 8 0 00-16 0', 'M12 13a5 5 0 100-10 5 5 0 000 10z'] },
  users: { paths: ['M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2', 'M9 11a4 4 0 100-8 4 4 0 000 8z', 'M22 21v-2a4 4 0 00-3-3.87', 'M16 3.13a4 4 0 010 7.75'] },
  settings: { paths: ['M4 7h6', 'M14 7h6', 'M4 17h10', 'M18 17h2', 'M10 4v6', 'M14 14v6'] },
  movie: { paths: ['M4 6h16v14H4z', 'M4 10h16', 'M8 6l2-3', 'M14 6l2-3'] },
  star: { paths: ['M12 3l2.8 5.7 6.2.9-4.5 4.4 1.1 6.2-5.6-3-5.6 3 1.1-6.2L3 9.6l6.2-.9L12 3z'] },
  key: { paths: ['M14 7a5 5 0 11-1.5 3.6L21 19v2h-3v-2h-2v-2h-2l-2.1-2.1', 'M7 7h.01'] },
  mail: { paths: ['M4 6h16v12H4z', 'M4 7l8 6 8-6'] },
  'message-square': { paths: ['M21 15a4 4 0 01-4 4H8l-5 3V7a4 4 0 014-4h10a4 4 0 014 4v8z'] },
  music: { paths: ['M9 18V5l11-2v13', 'M9 9l11-2', 'M6 21a3 3 0 100-6 3 3 0 000 6z', 'M17 19a3 3 0 100-6 3 3 0 000 6z'] },
  volume: { paths: ['M4 10v4h4l5 4V6l-5 4H4z', 'M16 9.5a4 4 0 010 5', 'M18.5 7a8 8 0 010 10'] },
  help: { paths: ['M9.1 9a3 3 0 115.8 1c0 2-3 2-3 4', 'M12 18h.01'], circles: [{ cx: 12, cy: 12, r: 9 }] },
  globe: { paths: ['M3 12h18', 'M12 3a15 15 0 010 18', 'M12 3a15 15 0 000 18'], circles: [{ cx: 12, cy: 12, r: 9 }] },
  warning: { paths: ['M12 3L2.8 20h18.4L12 3z', 'M12 9v4', 'M12 17h.01'] },
  play: { paths: ['M8 5l11 7-11 7V5z'] },
  grid: { paths: ['M4 4h6v6H4z', 'M14 4h6v6h-6z', 'M4 14h6v6H4z', 'M14 14h6v6h-6z'] },
  table: { paths: ['M4 5h16v14H4z', 'M4 10h16', 'M4 15h16', 'M10 5v14'] },
  list: { paths: ['M8 6h12', 'M8 12h12', 'M8 18h12', 'M4 6h.01', 'M4 12h.01', 'M4 18h.01'] },
  link: { paths: ['M10 13a5 5 0 007.1 0l2-2a5 5 0 00-7.1-7.1l-1.1 1.1', 'M14 11a5 5 0 00-7.1 0l-2 2A5 5 0 0012 20.1l1.1-1.1'] },
})

export const UI_BUTTON_ICON_ALIASES = Object.freeze({
  '+': 'plus',
  '＋': 'plus',
  '→': 'right',
  '↓': 'download',
  '⇣': 'download',
  '⌕': 'search',
  '✓': 'check',
  '!': 'warning',
  '!!': 'warning',
  pause: 'minus',
})

export function resolveUiIcon(name) {
  return UI_ICONS[name] || UI_ICONS.warning
}

export function resolveUiButtonIcon(name) {
  const normalizedName = UI_BUTTON_ICON_ALIASES[name] || name
  return UI_ICONS[normalizedName] ? normalizedName : ''
}
