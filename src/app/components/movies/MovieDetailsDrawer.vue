          ...
          <aside class="movie-drawer__panel" role="dialog" aria-modal="true" :aria-label="movie?.title || 'Карточка фильма'">
            ...
            <template v-else-if="details">
              ...
              <footer class="movie-drawer__footer">
                <MoviePlaybackLinks :key="`${details.mediaType}:${details.id}`" :movie="details" />
                <UiButton variant="secondary" :disabled="updating" @click="$emit('watched', details)">{{ watched ? 'Вернуть в «Хочу посмотреть»' : 'Просмотрено' }}</UiButton>
                <UiButton variant="secondary" :icon="planned ? '✓' : 'calendar'" @click="$emit('plan', details)">
                  {{ planned ? 'Убрать из календаря' : 'Запланировать' }}
                </UiButton>
                <UiButton :disabled="updating" :icon="saved ? '✓' : '+'" @click="$emit('toggle', details)">
                  {{ saved ? 'Убрать из списка' : 'Хочу посмотреть' }}
                </UiButton>
              </footer>
            </template>
          </aside>
