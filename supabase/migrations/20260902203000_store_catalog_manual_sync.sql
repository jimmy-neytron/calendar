...
-- enabled controls scheduling only. Existing callers default to scheduled mode.
comment on column public.store_catalog_sources.enabled is 'Автообновление по расписанию; ручной запуск доступен независимо от значения.';
drop function if exists public.save_store_catalog_snapshot(uuid,text,text,text,timestamptz,jsonb);
...
create or replace function public.save_store_catalog_snapshot(
  ...
  p_received_at timestamptz, p_products jsonb, p_manual boolean default false
...
revoke all on function public.save_store_catalog_snapshot(uuid,text,text,text,timestamptz,jsonb,boolean) from public, anon, authenticated;
grant execute on function public.save_store_catalog_snapshot(uuid,text,text,text,timestamptz,jsonb,boolean) to service_role;
commit;
