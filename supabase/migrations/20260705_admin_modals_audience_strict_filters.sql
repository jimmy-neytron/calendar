...
set audience = jsonb_set(coalesce(audience, '{}'::jsonb), '{mode}', '"targeted"'::jsonb, true),
    updated_at = now()
where coalesce(audience->>'mode', 'all') = 'all'
  and (
    jsonb_array_length(coalesce(audience->'userIds', '[]'::jsonb)) > 0
    or jsonb_array_length(coalesce(audience->'emails', '[]'::jsonb)) > 0
    or jsonb_array_length(coalesce(audience->'roles', '[]'::jsonb)) > 0
    or jsonb_array_length(coalesce(audience->'tiers', '[]'::jsonb)) > 0
  );
...
declare
  ...
  has_user_filter boolean := jsonb_array_length(coalesce(normalized_audience->'userIds', '[]'::jsonb)) > 0;
  has_email_filter boolean := jsonb_array_length(coalesce(normalized_audience->'emails', '[]'::jsonb)) > 0;
  ...
  has_any_filter boolean := has_user_filter or has_email_filter or has_role_filter or has_tier_filter;
begin
  if coalesce(normalized_audience->>'mode', 'all') = 'all' and not has_any_filter then
    return true;
  end if;
